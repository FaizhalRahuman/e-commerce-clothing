...
const CollViewByPrdId = () => {
    ...
    //----- Getting Image -----------------------------------------------------------------------------------------------------
        ...
        try{
            ...
            console.log("Error while Getting Product Collection Image Data in CollViewByPrdId.js:",err.message);
        }
    ...
    useEffect( () => {
        ...
        const prdCollections = async() => {
            ...
            try{
                ...
                setProdError(err.response.data)
...
const AllCollections = () => {
  ...
  useEffect(() => {
    ...
    <Box sx={{px:4,py:6,pt:12,backgroundColor:'ButtonShadow'}}>
        ...
        > PRODUCT COLLECTIONS </Typography>
        {prodError && <h4 style={{ color: 'red' }}>{prodError}</h4>}
        </Grid>
        ...
        >
            {prdCollections.map( (collection) => (
             <Grid item key={collection.collId}>
                ...
                />
                <Typography>Collection Id:{collection.collId}</Typography>
                </Box>
            </Grid>
            ))
        }
...
export default CollViewByPrdId;